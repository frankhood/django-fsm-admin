...
def fsm_submit_button(transition):
    ...
    single state.
    """
    fsm_field_name, button_value, transition_name, transition_css_classes = (
        transition
    )
    return {
...
def fsm_submit_row(context):
    ...
    def button_name(transition):
        if (
            hasattr(transition, "custom")
            and "button_name" in transition.custom
        ):
    ...
    def button_css_classes(transition):
        if (
            hasattr(transition, "custom")
            and "css_classes" in transition.custom
        ):
    ...
    for field, field_transitions in iter(transitions.items()):
        ctx["transitions"] += sorted(
            [
                (field, button_name(t), t.name, button_css_classes(t))
