...
class FSMTransitionMixin:
    ...
    * In the absence of specific transition permissions, the user must
    ...
    default_disallow_transition = not getattr(
        settings, "FSM_ADMIN_FORCE_PERMIT", False
    )
    ...
    def _fsm_get_transitions(self, obj, request, perms=None):
        ...
        for field in fsm_fields:
            ...
            transitions_generator = (
                getattr(obj, transitions_func)(user) if obj else []
            )
            transitions[field] = self._filter_admin_transitions(
                transitions_generator
            )
        return transitions
    ...
    def _filter_admin_transitions(self, transitions_generator):
        """
        Filter the given list of transitions, if their transition
        methods are declared as admin transitions.
        To allow a transition inside fsm_admin, add the parameter
        ...
        If the configuration setting `FSM_ADMIN_FORCE_PERMIT = True`
        then only transitions with `custom=dict(admin=True)` are allowed.
        Otherwise, if `FSM_ADMIN_FORCE_PERMIT = False` or
    ...
    def _do_transition(self, transition, request, obj, form, fsm_field_name):
        ...
        if available and trans_func:
            # Run the transition
            try:
                # Attempt to pass in the request and by argument
                # if using django-fsm-log
                trans_func(request=request, by=request.user)
            except TypeError:
                try:
                    # Attempt to pass in the by argument
                    # if using django-fsm-log
                    trans_func(by=request.user)
                except TypeError:
                    # If the function does not have a by attribute,
                    # just call with no arguments
                    trans_func()
            ...
            msg_dict.update(
                {"new_state": new_state, "status": messages.SUCCESS}
            )
        else:
